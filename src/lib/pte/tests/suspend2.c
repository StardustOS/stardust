 ...
 *
 * Cases Tested:
 * - parent supends and resumes child thread
 *
 * Description:
...
int pthread_test_suspend_resume2() {
    pthread_t child;
    void *result = 0;

    tprintk("suspend2: parent %i about to create child\n", pthread_self());
    assert(pthread_create(&child, NULL, mychild, NULL) == 0);

    sleep(100); // give child some time to run

    tprintk("suspend2: parent slept and about to suspend child %i\n", child);

    assert(pthread_kill(child, SIGUSR1) == 0);

    sleep(2000); // wait before resumeing child

    tprintk("suspend2: parent slept and about to resume child %i\n", child);
    assert(pthread_kill(child, SIGUSR1) == 0);

    tprintk("suspend2: parent about to join child %i and expect 0-valued result\n", child);
