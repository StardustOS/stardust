 ...
 *
 * Cases Tested:
 * - parent calls supend on child thread that is sleeping and resume on child thread that has woken
 *
 * Description:
...
int pthread_test_suspend_resume4() {
    pthread_t child;
    void *result = 0;

    tprintk("suspend4: parent %i about to create child\n", pthread_self());
    assert(pthread_create(&child, NULL, mychild, NULL) == 0);

    sleep(100); // give child some time to run and enter sleep

    tprintk("suspend4: parent slept and about to suspend child %i\n", child);

    assert(pthread_kill(child, SIGUSR1) == 0);

    sleep(2000); // wait before resumeing child

    tprintk("suspend4: parent slept and about to resume child %i\n", child);
    assert(pthread_kill(child, SIGUSR1) == 0);

    tprintk("suspend4: parent about to join child %i and expect 0-valued result\n", child);
